...
\begin{document}
    ...
    \begin{itemize}
        \item \textbf{前端基础与框架}：熟悉 HTML5、CSS3、JavaScript、TypeScript，掌握 Vue3、Vuex/Pinia、Next.js 等技术栈，能够完成页面组件化开发、状态管理、路由组织与复杂交互实现。
    ...
    \begin{itemize}
        \item \textbf{地图数据看板开发}：基于 Next.js 16 与腾讯地图 JSAPI GL 搭建数据看板模板，支撑 LBS 营销场景「需求 → 分析 → 交付」链路的数据呈现。
        \item \textbf{地图 Web 示例自动化（TypeScript）}：基于 Playwright + TypeScript 构建 JSAPI 地图 demo 与数据可视化示例的维护框架，覆盖 glAPI 129 实例与数据可视化 44 实例；单次截图由 58--103s 降至约 0.6s，130 条回归由 58.8min 降至 43.6min。
        \item \textbf{H5 发版质量保障}：完成【你用我赔 H5】发版需求测试分析，识别跨天状态机、端外分享、埋点与多端兼容性风险，建立 R1--R8 风险清单。
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{知识工程与语义检索}：设计页面/组件/坑点/操作四类节点知识图谱，基于 23 个研发仓库 UI 索引做同源静态推理，让 Agent 生成前先读懂研发代码；以 AST 扫描 1500+ 私有 helper，结合 jieba + BM25 实现公共方法语义检索与经验复用。
    \end{itemize}
