...
| SQL 执行成功率 70% -> 90%+ | 已定稿（2026-09-14，保留数字） | 口径：在约 200 条 Golden 评测集上统计“SQL 可成功执行且返回正确结果”的比例；语义正确性以执行结果与标准 SQL 对齐并辅以人工抽检 | 通过 SQL 试执行与自修正机制，显著降低语法错误和字段幻觉；在项目测试集中执行成功率提升至 90%+。 |
| 约 200 条 Golden Dataset | 可用但需确认 | 你确实参与构建或使用过该测试集，并能说明标注来源 | 构建面向真实业务问题的离线评估集，覆盖检索召回、事实一致性和 Text-to-SQL 执行结果。 |
...
- 指标：SQL 70% -> 90%+、约 200 条 Golden Dataset（2026-09-14 已定稿保留数字，口径见 §1.2）。
- 面试追问：
  - Text-to-SQL 最难的点是什么？
  - 如何评估 SQL 是否正确？
  - 如何避免查错库存/销售额？
