...
class ProjectScanner {
  ...
  async scanRootFlags() {
    ...
    try {
      ...
      this.result.warnings.push(
        `⚠ 无法读取项目根目录「${this.projectPath}」：${e.message}。` +
        `请确认你对该目录有读取权限，且路径未被占用或锁定。`
      );
      return;
    }
  ...
  async scanDirectory(basePath, depth) {
    ...
    try {
      ...
      this.result.warnings.push(
        `⚠ 跳过目录「${basePath}」（读取失败：${e.message}），继续扫描其余部分。`
      );
      return;
    }
...
async function main() {
  ...
  // 入口校验：路径不存在 / 不是目录时，给出明确的排查指引，而不是静默输出空结果
  try {
    const stat = fs.statSync(projectPath);
    if (!stat.isDirectory()) {
      console.error(
        `✖ 路径不是目录：${projectPath}\n` +
        `  本工具需要传入「项目根目录」，而不是某个文件。\n` +
        `  用法：node scan_project.js <项目目录> [--max-depth N]`
      );
      process.exit(1);
    }
    ...
    console.error(
      `✖ 找不到项目目录：${projectPath}\n` +
      `  请检查：\n` +
      `    1. 路径是否拼写正确（Windows 路径区分大小写，注意反斜杠）；\n` +
      `    2. 是否用了相对路径却不在当前工作目录（建议改用绝对路径）；\n` +
      `    3. 该目录是否已被移动或删除。\n` +
      `  用法：node scan_project.js <项目目录> [--max-depth N]`
    );
    process.exit(1);
  ...
  // 把警告单独打到 stderr，既不影响 stdout 的 JSON 解析，又让人一眼看到问题
  if (result.warnings && result.warnings.length) {
    console.error(
      `\n⚠ 本次扫描产生 ${result.warnings.length} 条警告（不影响主流程），详见下方 JSON 的 warnings 字段：`
    );
